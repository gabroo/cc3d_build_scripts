"""
This script runs on OSX and builds cc3d .
...
if __name__ == '__main__':
    ...
    cml_parser.add_argument("-d", "--dependency-channel", action="store", help="name of  conda dependency channel",
                            default='compucell3d')
    cml_parser.add_argument("--c-compiler", action="store", help="path to C compiler that properly supports OpenMP",
                            default='/usr/local/Cellar/gcc48/4.8.2/bin/gcc')
    cml_parser.add_argument("--cpp-compiler", action="store", help="path to C++ compiler that properly supports OpenMP",
                            default='/usr/local/Cellar/gcc48/4.8.2/bin/g++')
    ...
    conda_dependencies = 'qscintilla2 pyqtgraph webcolors jinja2 scipy vtk pyzmq cmake swig'
    ...
    if not isfile(c_compiler):
        print (
        'The specified C compiler: %s does not exist. '
        'Please use --c-compiler=... syntax to specify existing C compiler that properly supports OpenMP'%c_compiler)
        sys.exit(1)

    if not isfile(cpp_compiler):
        print (
        'The specified C++ compiler: %s does not exist. '
        'Please use --cpp-compiler=... syntax to specify existing C++ compiler that properly supports OpenMP'%c_compiler)
        sys.exit(1)
    ...
    # VTK_DIR = join('/Users/m/prerequisites/3.7.6/vtk', 'lib/cmake/vtk-6.3') # VTK we currently use on our OSX compilations

    PYQT_VERSION = 5

    cmake_config_command = [cmake_path, '-G', 'Unix Makefiles', '-DCMAKE_BUILD_TYPE:STRING=Release',
                            # '-DNO_OPENCL:BOOLEAN=ON',
                            '-DBUNDLE_PYTHON:BOOLEAN=OFF',
                            ...
                            '-DCMAKE_CXX_COMPILER:FILEPATH=' + cpp_compiler,
                            CC3D_SOURCE_PATH]
