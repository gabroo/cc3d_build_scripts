...
cd $CC3D_BUILD_SCRIPTS_GIT_DIR/linux/375

time run_and_watch_status BUILDING_CC3D_375_COMPILER_FARM_SCRIPT  ./build-debian-cc3d-375.sh -s=$CC3D_GIT_DIR -p=$install_path -c=$number_of_cpus

cd $CC3D_BUILD_SCRIPTS_GIT_DIR/linux/375/DebianPackageBuilder
...
python ./deb-pkg-builder-375.py -d $install_path -i ${install_path}_deb -v ${VERSION}
