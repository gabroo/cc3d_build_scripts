...
cd $CC3D_BUILD_SCRIPTS_GIT_DIR/linux/373

time run_and_watch_status BUILDING_CC3D_373_COMPILER_FARM_SCRIPT  ./build-debian-cc3d-373.sh -s=$CC3D_GIT_DIR -p=$install_path -c=$number_of_cpus

cd $CC3D_BUILD_SCRIPTS_GIT_DIR/linux/373/DebianPackageBuilder

echo "BEFORE RUNNING DEBIAN FILE"
pwd

# remove old deb packages
rm -rf ${install_path}_deb/*

python ./deb-pkg-builder-373.py -d $install_path -i ${install_path}_deb -v ${VERSION}
