...
then
  ...
  run_and_watch_status RR_CMAKE_CONFIG cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX:PATH=${INSTALL_PREFIX} -DBUILD_LLVM:BOOL=ON -DLLVM_CONFIG_EXECUTABLE:PATH=${LLVM_INSTALL_DIR}/bin/llvm-config -DBUILD_PYTHON:BOOL=ON -DTHIRD_PARTY_INSTALL_FOLDER:PATH=${INSTALL_PREFIX}  -DCMAKE_BUILD_TYPE:STRING=Release $SOURCE_ROOT 
