# example command:
# c:\miniconda32\python .\win_cc3d_builder_gpu_376.py  -p D:/install_projects/3.7.6-gpu-32bit -s D:/CC3D_GIT  -i D:/CC3D_FILES_SVN/binaries/3.7.6/windows -v 3.7.6.0
...
win_installer_creator = 'win_cc3d_installer_creator_376.py'
...
parser = OptionParser()
parser.add_option("-p", "--prefix", dest="prefix", action="store", type="string",
                  default='D:/install_projects/' + version_string + '-gpu', help="CC3D installation directory")
parser.add_option("-s", "--source-root", dest="source_root", action="store", type="string", default='D:/CC3D_GIT',
                  help="CC3D git repository")
parser.add_option("-c", "--cores", dest="cores", action="store", type="int", default=1,
                  help="Number of compilation threads for make")
parser.add_option('-i', '--installer-dir', dest='installer_dir', action='store', type='string', default='',
                  help='Location of the place where to store installer')
parser.add_option("-v", "--version", dest="version", action="store", type="string", default=version_string + '.0',
                  help='version of installer')

parser.add_option("--cc3d", action="store_true", default=False, dest="cc3d",
                  help='this option picks CompuCell3D to be compiled. Using it will require all other project files to be set individually')
parser.add_option("--bionet", action="store_true", default=False, dest="bionet",
                  help='this option picks BionetSolver to be compiled. Using it will require all other project files to be set individually')
parser.add_option("--celldraw", action="store_true", default=False, dest="celldraw",
                  help='this option picks BionetSolver to be compiled. Using it will require all other project files to be set individually')
parser.add_option("--rr", action="store_true", default=False, dest="rr",
...
PYQT_VERSION = 5
WIN_DEPENDENCIES_ROOT = abspath('d:/prerequisites_2017/32bit')
...
if BUILD_CC3D:
    ...
    subprocess.call([CMAKE_PATH, '-G', CMAKE_GENERATOR_NAME, '-DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo',
...
if BUILD_INSTALLER:
    ...
    INSTALLER_NAME = os.path.abspath(
        os.path.join(INSTALLER_DIR, 'CompuCell3D-32bit-gpu-setup-' + version + 'v' + revisionNumber + '.exe'))

    os.chdir(CURRENT_DIR)
    subprocess.call(
        ['python', WIN_INSTALLER_CREATOR, '-d', INSTALL_PREFIX, '-v', INSTALLER_VERSION, '-i', INSTALLER_NAME])
