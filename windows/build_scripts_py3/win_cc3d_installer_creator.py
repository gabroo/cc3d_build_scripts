...
NSIS_EXE_PATH = r'C:\Program Files (x86)\NSIS\makensis.exe'
...
parser = OptionParser()
parser.add_option("-d", "--directory", dest="dirname", action="store", type="string",
                  help="CC3D installation directory")
parser.add_option("-i", "--installer-name", dest="installer_name", action="store", default='', type="string",
                  help="full installer name")
parser.add_option("-v", "--version", dest="version", action="store", type="string", help="CC3D version", default='')
parser.add_option("-t", "--template", dest="template", action="store", type="string", help="installer template",
                  default='')
...
if INSTALLER_NAME == '':
    ...
    INSTALLER_NAME = os.path.abspath(
        os.path.join(cwd, 'CompuCell3D-x64-setup-' + version + 'v' + revisionNumber + '.exe'))
else:
...
inFile = open(INSTALLER_TEMPLATE_NAME, 'r')
