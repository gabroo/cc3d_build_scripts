# example command:
# python .\win_cc3d_builder_no_gpu_373.py  -p D:\install_projects\3.7.3_no_gpu -s D:\CC3D_GIT  -i D:\CC3D_FILES_SVN\binaries\3.7.3\windows -v 3.7.3.0
...
parser = OptionParser()
parser.add_option("-p", "--prefix", dest="prefix",action="store", type="string",default='D:/install_projects/3.7.3_no_gpu', help="CC3D installation directory")
...
parser.add_option("-v", "--version", dest="version",action="store", type="string", default='3.7.3.0',help='version of installer')
...
WIN_INSTALLER_CREATOR='win_cc3d_installer_creator_no_gpu_373.py'
...
if BUILD_CC3D:    
    ...
    if DO_NOT_USE_OPENCL:
        
        subprocess.call([CMAKE_PATH,'-G', CMAKE_GENERATOR_NAME,'-DNO_OPENCL:BOOLEAN=ON','-DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo','-DCMAKE_INSTALL_PREFIX:PATH='+INSTALL_PREFIX,'-DCOMPUCELL3D_A_MAJOR_VERSION:STRING='+str(MAJOR_VERSION),'-DCOMPUCELL3D_B_MINOR_VERSION:STRING='+str(MINOR_VERSION),'-DCOMPUCELL3D_C_BUILD_VERSION:STRING='+str(BUILD_VERSION),'-DWINDOWS_DEPENDENCIES:PATH='+WIN_DEPENDENCIES_ROOT, CC3D_SOURCE_PATH ])
    else:    
...
if  BUILD_INSTALLER:    
    ...
    INSTALLER_NAME=os.path.abspath(os.path.join(INSTALLER_DIR,'setup-no-gpu-'+version+'v'+revisionNumber+'.exe'))
